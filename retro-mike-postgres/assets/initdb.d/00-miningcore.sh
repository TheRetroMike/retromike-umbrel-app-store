...
echo "[initdb] provisioning miningcore role/db/schema ..."
...
DB_EXISTS="$(psql -tAc "SELECT 1 FROM pg_database WHERE datname='miningcore'" --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" || true)"
if [ "${DB_EXISTS}" != "1" ]; then
  createdb --username "$POSTGRES_USER" --owner miningcore miningcore
fi

# Create schema + tables + indexes (idempotent)
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "miningcore" <<'SQL'
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_namespace WHERE nspname='miningcore') THEN
    EXECUTE 'CREATE SCHEMA miningcore AUTHORIZATION miningcore';
  END IF;
END
$$;

ALTER ROLE miningcore SET search_path TO miningcore,public;
SET search_path TO miningcore,public;
...
echo "[initdb] miningcore provisioning done"
